...
function normalizeBlockingLevel(input) {
  const level = String(input || 'B3').toUpperCase();
...
router.post('/api/groups/:groupId/petitions', requireAuth, requireGroupMember, async (req, res) => {
  try {
    const startMs = parseEpochMs(req.body?.start);
    const endMs = parseEpochMs(req.body?.end);
    const title = typeof req.body?.title === 'string' ? req.body.title.trim() : '';
    const blockingLevel = normalizeBlockingLevel(req.body?.blocking_level ?? req.body?.blockingLevel ?? 'B3');
...
router.get('/api/groups/:groupId/petitions', requireAuth, requireGroupMember, async (req, res) => {
  try {
...
router.get('/api/petitions', requireAuth, async (req, res) => {
  try {
...
router.post('/api/petitions/:petitionId/respond', requireAuth, async (req, res) => {
  try {
...
router.delete('/api/petitions/:petitionId', requireAuth, async (req, res) => {
  try {
    const petitionId = Number(req.params.petitionId);
    if (!Number.isInteger(petitionId) || petitionId <= 0) {
      return sendApiError(res, 400, 'Invalid petitionId');
    ...
    const result = await db.deleteFailedPetition({
      petitionId,
      userId: req.userId
    });
