...
export default function PetitionActionModal({
  ...
  const isCreator = Number(currentUserId) === creatorId;
  const canDelete = isCreator && status === 'FAILED';
  const acceptDisabled = submitting || currentUserResponse === 'ACCEPTED' || currentUserResponse === 'ACCEPT';
  const declineDisabled = submitting || currentUserResponse === 'DECLINED' || currentUserResponse === 'DECLINE';
        ...
        <div className="petition-modal-actions">
          {isCreator ? (
            <button
              ...
              title={!canDelete ? 'Creators can only delete FAILED petitions.' : 'Delete petition'}
            >
              Delete
            </button>
          ) : (
